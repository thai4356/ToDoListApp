...
public class TaskRepositoryImpl implements TaskRepositoryCustom {
    ...
    public List<Task> search(Integer teamId, Integer assigneeId, String status, String priority,
        ...
        if (status != null) q = q.where(t.status.eq(Task.Status.valueOf(status.toLowerCase())));
        if (priority != null) q = q.where(t.priority.eq(Task.Priority.valueOf(priority.toLowerCase())));
        if (dueFrom != null) q = q.where(t.dueDate.goe(dueFrom));
        if (dueTo != null) q = q.where(t.dueDate.loe(dueTo));
