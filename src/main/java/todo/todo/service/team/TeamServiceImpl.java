...
import todo.todo.repository.teamInvite.TeamInviteRepository;
...
public class TeamServiceImpl extends BaseService implements TeamService {
        ...
        public TeamServiceImpl(SendEmailServiceImpl sendEmailService, TeamInviteRepository teamInviteRepository,
        ...
        public TeamDetailRes UpdateTeam(AddTeamBaseReq request, int foundId, int currentUserId) {
                ...
                List<TeamMemberRes> memberDtos = teamMemberRepository.findActiveMemberDtosByTeamId(team.getId());
                return TeamDetailRes.builder()
                                .id(team.getId())
                                .name(team.getName())
                                .description(team.getDescription())
        ...
        @Override
        public TeamMemberRes inviteMember(int teamId, String email, int currentUserId) {

                Team team = teamRepository.findById(teamId)
                                .orElseThrow(() -> new RuntimeException("Team not found"));

                TeamMember current = teamMemberRepository.findByUserIdAndTeamId(currentUserId, teamId);

                if (current == null || current.getRole() == TeamMember.Role.member) {
                        throw new RuntimeException("No permission to invite");
                }

                User invited = userRepository.getUserByEmail(email);
                if (invited == null)
                ...
                invite.setInvitedUserId(invited.getId());
                ...
                String link = "http://localhost:6868/api/v1/team/invite/accept?token=" + token;

                EmailDetail detail = new EmailDetail();
                detail.setRecipient(invited.getEmail());
                detail.setSubject("Team invitation");
                detail.setMsgBody(
                                "Bạn được mời vào team: " + team.getName() +
                                                "<br>Nhấn vào link để xác nhận: <a href=\"" + link
                                                + "\">Xác nhận tham gia</a>");

                sendEmailService.sendSimpleMail(detail);
        ...
        public String acceptInvite(String token) {
                ...
                TeamInvite invite = teamInviteRepository
                                .findByTokenAndAcceptedFalse(token)
                                .orElseThrow(() -> new BusinessException("Token không hợp lệ hoặc đã dùng"));

                Team team = teamRepository.findById(invite.getTeamId())
                                .orElseThrow(() -> new BusinessException("Team không tồn tại"));

                User user = userRepository.findById(invite.getInvitedUserId())
                ...
                TeamInvite InviteUpdate = teamInviteRepository.findByToken(token);
                InviteUpdate.setAccepted(true);
                InviteUpdate.setUpdatedAt(new Date());
                teamInviteRepository.save(InviteUpdate);
